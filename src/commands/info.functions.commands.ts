...
export async function menuCommand(client: WASocket, botInfo: Bot, message: Message, group?: Group){
    const isMenuBlocked = botInfo.block_cmds.includes('menu')
    
    if (isMenuBlocked && !message.isBotAdmin) {
        const replyText = "🔒 **Menu Restrito**\n\n" +
                         "❌ O comando !menu está temporariamente restrito.\n" +
