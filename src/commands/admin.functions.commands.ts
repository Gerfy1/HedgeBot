...
export async function blockmenuCommand(client: WASocket, botInfo: Bot, message: Message, group?: Group) {
    const botController = new BotController()
    
    await botController.setBlockedCommands(botInfo.prefix, ['menu'], 'add')
    
    const replyText = "✅ Comando !menu foi **BLOQUEADO** para usuários comuns.\n\n" +
                     "📝 Apenas admins do bot podem usar agora.\n" +
                     "🔓 Use `!desbloquearmenu` para liberar novamente."
    
    await waUtil.replyText(client, message.chat_id, replyText, message.wa_message, {expiration: message.expiration})
}

export async function unblockmenuCommand(client: WASocket, botInfo: Bot, message: Message, group?: Group) {
    const botController = new BotController()
    
    await botController.setBlockedCommands(botInfo.prefix, ['menu'], 'remove')
    
    const replyText = "🔓 Comando !menu foi **LIBERADO** para todos os usuários."
