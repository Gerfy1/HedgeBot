...
export async function sendLinkWithPreview(client: WASocket, chatId: string, text: string, options?: MessageOptions){
    await updatePresence(client, chatId, "composing")
...
export async function sendTwitchLinkWithPreview(client: WASocket, chatId: string, text: string, options?: MessageOptions){
    await updatePresence(client, chatId, "composing")
...
export async function sendLinkWithRetryPreview(client: WASocket, chatId: string, text: string, options?: MessageOptions, maxRetries: number = 3){
    await updatePresence(client, chatId, "composing")
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`🔗 Tentativa ${attempt}/${maxRetries} de envio do link com preview...`)
            ...
            console.log(`✅ Link enviado com sucesso na tentativa ${attempt}`)
            return result
            
        } catch (error) {
            console.log(`⚠️ Falha na tentativa ${attempt}: ${error}`)
            
            if (attempt === maxRetries) {
                console.log(`❌ Enviando link sem preview após ${maxRetries} tentativas`)
                return client.sendMessage(chatId, {text, linkPreview: null}, {ephemeralExpiration: options?.expiration})
            }
        }
    }
